import { MetricsStoreObject, PrometheusMetric, StoreOperationOptions } from '../index.dto';
import { Prometheus } from '@promster/metrics';
import { TimerMetric } from './timer-metric';
import { PinoLogger } from 'src/utils/logger/pino-logger';
...
export class MetricsStoreService {
  ...
  public inc(name: string, { labels, value }: StoreOperationOptions = this.defaultOptions): void {
    try {
      ...
      PinoLogger.MetricLogger.error(`Cannot perform operation on metric ${name} due to "${error.message}"`);
    }
  }
